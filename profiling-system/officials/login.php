...
function _log_login(mysqli $conn, int $uid, string $utype, string $uname, string $fname): int
{
    ...
    if (preg_match('/Android ([0-9.]+)/i', $ua, $m))
        $os = 'Android ' . $m[1];
    elseif (preg_match('/OS ([0-9_]+) like Mac OS/i', $ua, $m))
            ...
            if (!$matched) {
                $stmt = $conn->prepare(
                    "SELECT id, username, password, first_name, surname,
                            user_role, account_status, is_purok_president, purok, barangay
                    ...
                    if (password_verify($password, $user['password'])) {
                        if ($user['account_status'] === 'suspended') {
                            ...
                            if ((int) $user['is_purok_president'] === 1) {
                                ...
                                // ── Normal resident login is disabled ────────────
                                $error_type = 'resident_disabled';
                            }
                        }
                    } else {
                        $error_type = 'invalid';
    ...
    <div class="login-wrap">
        <div class="login-brand">
            ...
            <div class="login-brand-content">
                ...
                <div>
                    <div class="login-feature-item"><span class="login-feature-dot"></span> Centralized Resident Records
                    </div>
                    <div class="login-feature-item"><span class="login-feature-dot"></span> Household Connectivity
                        Mapping</div>
                    <div class="login-feature-item"><span class="login-feature-dot"></span> Advanced Demographic
                        Analytics</div>
                    <div class="login-feature-item"><span class="login-feature-dot"></span> Official Accountability
                        Tracking</div>
                </div>
            </div>
        </div>
        ...
        <div class="login-form-panel">
            ...
            <div class="login-card">
                <div class="text-center mb-4">
                    <img src="image/logo.jpg" alt="Logo" class="mb-3"
                ...
                <?php elseif ($error_type === 'resident_disabled'): ?>
                    <div class="alert alert-warning small mb-3"
                        style="border-radius:10px; background-color:#fffdf3; border-color:#ffecb5;">
                        <div class="d-flex align-items-center">
                            <i class="bi bi-slash-circle text-danger fs-4 me-3"></i>
                            <div>
                                <strong class="text-danger">Resident Login Disabled</strong><br>
                                <span class="text-muted" style="font-size:0.8rem;">The resident portal is currently closed
                                    for standard users. Only authorized staff and officials may log in.</span>
                            </div>
                        </div>
                    </div>
                <?php endif; ?>
                ...
                <?php if (!$just_registered || $error_type): ?>
                    <form method="POST" autocomplete="off">
                        ...
                        <div class="mb-3">
                            ...
                            <div class="input-group">
                                ...
                                <input type="text" class="form-control" name="username" placeholder="Username" required
                                    autofocus value="<?= htmlspecialchars($_POST['username'] ?? '') ?>" <?= $login_disabled ? 'disabled' : '' ?>>
                            </div>
                        </div>
                        <div class="mb-3">
                            ...
                            <div class="input-group">
                                ...
                                <input type="password" class="form-control" name="password" id="pwdField"
                                    placeholder="Password" required <?= $login_disabled ? 'disabled' : '' ?>>
                        ...
                        <div class="d-flex justify-content-between align-items-center mb-4">
                            ...
                            <?php endif; ?>
                            <a href="<?= URL_REGISTER ?>" class="small text-decoration-none fw-bold"
                                style="color: var(--brand-secondary);">Register Resident</a>
                        </div>
                ...
                <?php else: ?>
                    <div class="text-center">
                        <a href="<?= URL_LOGIN ?>" class="btn btn-outline-primary btn-sm rounded-pill mt-2">← Back to
                            Login</a>
                    </div>
                <?php endif; ?>
            </div>
