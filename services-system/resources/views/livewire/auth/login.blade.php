    ...
    public function login(): void
    {
        ...
        if (!$authenticated) {
            ...
            if ($resident && $this->verifyPassword($this->password, $resident->password)) {
                // Check if resident is 18 or above
                $age = !empty($resident->birthdate) ? \Carbon\Carbon::parse($resident->birthdate)->age : 0;
                if ($age < 18) {
                    throw ValidationException::withMessages([
                        'username' => 'You must be 18 years old and above to login.',
                    ]);
                }
        ...
        if (!$authenticated) {
            ...
            if ($user && Hash::check($this->password, $user->password)) {
                if ($user->is_resident) {
                    ...
                    $age = ($resident && !empty($resident->birthdate)) ? \Carbon\Carbon::parse($resident->birthdate)->age : 0;
                    if ($age < 18) {
                        throw ValidationException::withMessages([
                            'username' => 'You must be 18 years old and above to login.',
                        ]);
