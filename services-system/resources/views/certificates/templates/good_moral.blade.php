...
<head>
    <meta charset="UTF-8">
    <title>Barangay Certification (Good Moral)</title>
    <style>
        @font-face {
        ...
        .sig-title { font-family: "Century Gothic", Arial, sans-serif; font-weight: bold; font-size: 10pt; }
        .footer-note { margin-top: 40px; font-size: 8pt; font-style: italic; }
    </style>
</head>
<body>
    ...
    <div class="title-container">
        <h1 class="certificate-title">CERTIFICATION</h1>
    </div>
    ...
    <div class="content">
        <p>
            <strong>THIS IS TO CERTIFY</strong> Mr./Ms./Mrs.
            <strong>{{ $resident->first_name }} {{ $resident->middle_name }} {{ $resident->surname ?? $resident->last_name }}{{ $resident->suffix ? ', ' . $resident->suffix : '' }}</strong>,
            {{ $resident->age }} yrs. old, resident of {{ $resident->purok->purok_name ?? $resident->purok ?? '' }}, Barangay Sto. Rosario, Magallanes, Agusan del Norte,
            is a bona fide resident of this barangay and is personally known to me as a law-abiding citizen with good moral character.
        </p>

        <p>
            <strong>This certification is being issued upon the request of
            {{ $resident->first_name }} {{ $resident->middle_name }} {{ $resident->surname ?? $resident->last_name }}
            for {{ $request->purpose ?? 'whatever purpose it may serve' }}.</strong>
        </p>

        <p>
            <strong><u>Issued this {{ now()->format('j') }}{{ now()->format('S') }} day of {{ now()->format('F') }}, {{ now()->year }} at Barangay Sto. Rosario, Magallanes, Agusan del Norte.</u></strong>
        </p>
    </div>

    {{-- ─── E-SIGNATURE + NAME ──────────────────────── --}}
    <div class="signature-block">
        @if(file_exists(storage_path('app/public/logos/esign_good_moral.jpeg')))
            <img src="{{ storage_path('app/public/logos/esign_good_moral.jpeg') }}" alt="" class="esign-img"><br>
        @endif
