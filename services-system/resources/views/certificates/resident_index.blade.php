...
<script>
    ...
    searchInput.addEventListener('input', function () {
        ...
        <div>
            <a href="{{ route('certificates.create') }}" class="px-4 py-2 text-sm font-medium text-white bg-indigo-600 rounded-md hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
                Request Certificate
            </a>
        </div>
    </div>
        ...
        <table class="min-w-full text-sm dark:bg-[#0f1e2e]">
            <thead class="bg-neutral-50 dark:bg-[#15233b] dark:text-[#e5e7eb]">
                <tr>
                    ...
                    <th class="px-4 py-2 text-left font-medium dark:text-[#e5e7eb]">Remarks</th>
                    ...
                    <tr class="border-t border-neutral-200 dark:border-neutral-700 dark:bg-[#0f1e2e]">
                        ...
                        <td class="px-4 py-2 dark:text-[#e5e7eb]">
                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium
                                ...
                                @elseif(strtolower($paymentStatus) === 'rejected') bg-red-100 text-red-800 dark:bg-red-900 dark:text-red-200
                                @else bg-yellow-100 text-yellow-800 dark:bg-yellow-900 dark:text-yellow-200 @endif">
                                {{ $paymentStatus }}
                            </span>
                        </td>
                        <td class="px-4 py-2 dark:text-[#e5e7eb] max-w-xs truncate" title="{{ $request->payment_remarks }}">
                            @if(strtolower($paymentStatus) === 'rejected' && $request->payment_remarks)
                                <span class="text-sm text-red-600 dark:text-red-400">{{ $request->payment_remarks }}</span>
                            @else
