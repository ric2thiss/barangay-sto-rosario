    ...
    public function exportReport(Request $request)
{
    ...
    exec(sprintf('%s --headless --convert-to pdf --outdir %s %s 2>&1',
        $soffice, escapeshellarg($outputDir), escapeshellarg($tempDocx)
    ), $output, $exitCode);

    @unlink($tempDocx);

    if ($exitCode !== 0) abort(500, 'PDF conversion failed: ' . implode(' ', $output));
