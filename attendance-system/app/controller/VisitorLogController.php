...
class VisitorLogController
{
    ...
    public function store(array $data): array
    {
        ...
        try {
            ...
            // 1. If a booking_id is provided, only allow ONE log per booking_id
            $bookingId = $logData['booking_id'] ?? null;
            if (!empty($bookingId)) {
                $existing = $this->visitorLogRepository->findByBookingId($bookingId);
                if ($existing) {
                    return [
                        'success' => false,
                        'already_logged' => true,
                        'error' => 'This request/appointment has already been logged.'
                    ];
                }
            }

            // 2. For residents without a booking, prevent duplicate same-day + same-purpose
            if ($bookingId === null && $logData['resident_id'] !== null) {
                $existingToday = $this->visitorLogRepository->findTodayLogByResident(
                    (int) $logData['resident_id'],
                    $logData['purpose']
                );
                if ($existingToday) {
                    return [
                        'success' => false,
                        'already_logged' => true,
                        'error' => 'This resident has already been logged today for the same purpose.'
                    ];
                }
            }
