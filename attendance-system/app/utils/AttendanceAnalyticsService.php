...
require_once __DIR__ . '/ProfilingPdo.php';
...
class AttendanceAnalyticsService {
    ...
    public function buildReport(array $range, array $filters = []): array {
        ...
        if ($statusFilter !== '' && $statusFilter !== 'all') {
            ...
            $rows = array_values(array_filter($rows, function ($row) use ($sf) {
        ...
        // Search filter: narrow by employee name substring (case-insensitive)
        $searchFilter = isset($filters['search']) ? trim((string) $filters['search']) : '';
        if ($searchFilter !== '') {
            $searchLower = mb_strtolower($searchFilter, 'UTF-8');
            $rows = array_values(array_filter($rows, function ($row) use ($searchLower) {
                return mb_strpos(mb_strtolower($row['employee_name'], 'UTF-8'), $searchLower) !== false;
            }));
        }
