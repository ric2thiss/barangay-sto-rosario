...
/**
 * Deployment configuration: Load from .env (Recommended) or deploy.config.json (Legacy).
 */
...
if (is_file($__envPath) && is_readable($__envPath)) {
    $parsedEnv = parse_ini_file($__envPath);
    if ($parsedEnv !== false) {
        $__env = $parsedEnv;
    }
}

// 2. Fallback to deploy.config.json if it exists (legacy support)
...
if (!defined("VERIFICATION_SECRET_KEY")) {
    define("VERIFICATION_SECRET_KEY", (string) ($__env['VERIFICATION_SECRET_KEY'] ?? $__deploy['security']['verificationSecretKey'] ?? "MY_SECRET_KEY"));
}
 ...
 */
if (!defined("BASE_URL")) {
    $configuredBaseUrl = trim((string) ($__env['BASE_URL'] ?? $__deploy['baseUrl'] ?? ''));
...
if (!defined("WEBSOCKET_HOST")) {
    define("WEBSOCKET_HOST", (string) ($__env['WEBSOCKET_HOST'] ?? $__deploy['websocket']['host'] ?? "localhost"));
}
if (!defined("WEBSOCKET_PORT")) {
    define("WEBSOCKET_PORT", (int) ($__env['WEBSOCKET_PORT'] ?? $__deploy['websocket']['port'] ?? 8081));
}
...
if (!defined("DB_HOST")) {
    define("DB_HOST", (string) ($__env['DB_HOST'] ?? $__deploy['database']['host'] ?? "localhost"));
}
if (!defined("DB_NAME")) {
    define("DB_NAME", (string) ($__env['DB_NAME'] ?? $__deploy['database']['name'] ?? "attendance-system"));
}
if (!defined("DB_USER")) {
    define("DB_USER", (string) ($__env['DB_USER'] ?? $__deploy['database']['user'] ?? "root"));
}
if (!defined("DB_PASS")) {
    define("DB_PASS", (string) ($__env['DB_PASS'] ?? $__deploy['database']['pass'] ?? ""));
}
if (!defined("DB_CHARSET")) {
    define("DB_CHARSET", (string) ($__env['DB_CHARSET'] ?? $__deploy['database']['charset'] ?? "utf8mb4"));
}

// External profiling system database (read-only for attendance-system)
if (!defined("PROFILING_DB_NAME")) {
    define("PROFILING_DB_NAME", (string) ($__env['PROFILING_DB_NAME'] ?? $__deploy['databases']['profiling'] ?? "profiling-system"));
}
if (!defined("PROFILING_DB_USER")) {
    define("PROFILING_DB_USER", (string) ($__env['PROFILING_DB_USER'] ?? DB_USER));
}
if (!defined("PROFILING_DB_PASS")) {
    define("PROFILING_DB_PASS", (string) ($__env['PROFILING_DB_PASS'] ?? DB_PASS));
}
...
if (!defined("BARANGAY_SERVICES2_DB_NAME")) {
    define("BARANGAY_SERVICES2_DB_NAME", (string) ($__env['BARANGAY_SERVICES2_DB_NAME'] ?? $__deploy['databases']['barangayServices2'] ?? "barangay_services2"));
}
if (!defined("BARANGAY_SERVICES2_DB_USER")) {
    define("BARANGAY_SERVICES2_DB_USER", (string) ($__env['BARANGAY_SERVICES2_DB_USER'] ?? DB_USER));
}
if (!defined("BARANGAY_SERVICES2_DB_PASS")) {
    define("BARANGAY_SERVICES2_DB_PASS", (string) ($__env['BARANGAY_SERVICES2_DB_PASS'] ?? DB_PASS));
}

// LGUMS scheduling database (read-only; schedule_events). Same MySQL server as attendance-system.
if (!defined("LGUMS_DB_NAME")) {
    define("LGUMS_DB_NAME", (string) ($__env['LGUMS_DB_NAME'] ?? $__deploy['databases']['lgums'] ?? "lgums"));
}
if (!defined("LGUMS_DB_USER")) {
    define("LGUMS_DB_USER", (string) ($__env['LGUMS_DB_USER'] ?? DB_USER));
}
if (!defined("LGUMS_DB_PASS")) {
    define("LGUMS_DB_PASS", (string) ($__env['LGUMS_DB_PASS'] ?? DB_PASS));
}
...
if (!defined("LGUMS_SCHEDULE_EVENT_DATE_COL")) {
    define("LGUMS_SCHEDULE_EVENT_DATE_COL", "start_datetime");
}
