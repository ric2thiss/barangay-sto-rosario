...
// Create temp table for ProfilingPdo data
...
if ($profilingPdo) {
    $resStmt = $profilingPdo->query("SELECT id, purok FROM `residents`");
    $resRows = $resStmt->fetchAll(PDO::FETCH_ASSOC) ?: [];
    if (!empty($resRows)) {
        $insRes = $pdo->prepare("INSERT INTO `_tmp_residents` (id, purok) VALUES (?, ?)");
        foreach ($resRows as $row) {
            $insRes->execute([$row['id'], $row['purok'] ?? null]);
        }
    }
}
...
// Dynamic soft-delete filter — safe if column doesn't exist
$delFilterPlain = SchemaColumnCache::visitorLogsHasDeletedAt() ? 'deleted_at IS NULL AND ' : '';
$delFilterVl    = SchemaColumnCache::visitorLogsHasDeletedAt() ? 'vl.deleted_at IS NULL AND ' : '';
 ...
 */
function buildVisitorTrend(PDO $pdo, string $filter, DateTime $startDate, DateTime $endDate, DateTime $now, string $cardPlain, string $delFilter = ''): array {
    $labels = [];
    $counts = [];
    ...
    if ($filter === 'today') {
        for ($hour = 0; $hour < 24; $hour++) {
            ...
            $stmt = $pdo->prepare("
                ...
                WHERE {$delFilter}created_at >= ? AND created_at <= ? {$cardPlain}
            ");
        ...
        for ($i = 0; $i < 7; $i++) {
            ...
            $stmt = $pdo->prepare("
                SELECT COUNT(*) AS c FROM visitor_logs
                WHERE {$delFilter}DATE(created_at) = ? {$cardPlain}
            ");
...
try {
    if (!$skipCharts) {
        $trend = buildVisitorTrend($pdo, $filter, $startDate, $endDate, $now, $cardPlain, $delFilterPlain);
        ...
        FROM visitor_logs
        WHERE {$delFilterPlain}created_at >= ? AND created_at <= ? {$cardPlain}
    ");
    ...
    ");
        ...
        WHERE {$delFilterVl}vl.created_at >= ? AND vl.created_at <= ? {$cardVl}
