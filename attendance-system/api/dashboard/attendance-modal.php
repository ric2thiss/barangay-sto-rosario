...
// Create temp tables for ProfilingPdo data
require_once __DIR__ . '/../../app/utils/ProfilingPdo.php';
$pdo->exec("CREATE TEMPORARY TABLE IF NOT EXISTS `_tmp_barangay_official` (
    `id` INT PRIMARY KEY,
...
$pdo->exec("CREATE TEMPORARY TABLE IF NOT EXISTS `_tmp_residents` (
    `id` INT PRIMARY KEY,
    `first_name` VARCHAR(100),
    `surname` VARCHAR(100)
) ENGINE=MEMORY");
...
if ($profilingPdo) {
    // Populate officials
    $offStmt = $profilingPdo->query("SELECT id, first_name, middle_name, surname FROM `barangay_official`");
    $offRows = $offStmt->fetchAll(PDO::FETCH_ASSOC) ?: [];
    if (!empty($offRows)) {
        $insOff = $pdo->prepare("INSERT INTO `_tmp_barangay_official` (id, first_name, middle_name, surname) VALUES (?, ?, ?, ?)");
        foreach ($offRows as $row) {
            $insOff->execute([$row['id'], $row['first_name'] ?? null, $row['middle_name'] ?? null, $row['surname'] ?? null]);
        }
    }
    
    // Populate residents
    $resStmt = $profilingPdo->query("SELECT id, first_name, surname FROM `residents`");
    $resRows = $resStmt->fetchAll(PDO::FETCH_ASSOC) ?: [];
    if (!empty($resRows)) {
        $insRes = $pdo->prepare("INSERT INTO `_tmp_residents` (id, first_name, surname) VALUES (?, ?, ?)");
        foreach ($resRows as $row) {
            $insRes->execute([$row['id'], $row['first_name'] ?? null, $row['surname'] ?? null]);
        }
    }
}

$residents = '`_tmp_residents`';
$barangayTable = '`_tmp_barangay_official`';

$employeeRepository = new EmployeeRepository($pdo);
$officialsTableUsable = true; // Always true because we create it
try {
...
$delFilterA = SchemaColumnCache::attendancesHasDeletedAt() ? 'a.deleted_at IS NULL AND ' : '';
$delFilterA0 = SchemaColumnCache::attendancesHasDeletedAt() ? 'a0.deleted_at IS NULL AND ' : '';
$delFilterA1 = SchemaColumnCache::attendancesHasDeletedAt() ? 'a1.deleted_at IS NULL AND ' : '';
...
try {
    if ($filter === 'today') {
        for ($hour = 0; $hour < 24; $hour++) {
            ...
            $stmt = $pdo->prepare("SELECT COUNT(DISTINCT employee_id) AS c FROM attendances WHERE {$delFilter}created_at >= ? AND created_at <= ?");
            $stmt->execute([$hourStart, $hourEnd]);
            $trendPresent[] = (int) ($stmt->fetchColumn() ?: 0);
            $trendAbsent[] = 0;

            $stmt = $pdo->prepare("SELECT COUNT(*) AS c FROM attendances a WHERE {$delFilterA}a.created_at >= ? AND a.created_at <= ? AND {$LATE_WHERE}");
            $stmt->execute([$hourStart, $hourEnd]);
            $trendLate[] = (int) ($stmt->fetchColumn() ?: 0);

            $stmt = $pdo->prepare("SELECT COUNT(*) AS c FROM attendances a WHERE {$delFilterA}a.created_at >= ? AND a.created_at <= ? AND {$OT_WHERE}");
            $stmt->execute([$hourStart, $hourEnd]);
        ...
        for ($i = 0; $i < 7; $i++) {
            ...
            $stmt = $pdo->prepare("SELECT COUNT(DISTINCT employee_id) AS c FROM attendances WHERE {$delFilter}DATE(created_at) = DATE(?)");
    ...
    if ($card === 'present') {
        ...
        if ($officialsTableUsable) {
            ...
            WHERE NOT EXISTS (
                ...
                WHERE {$delFilterA}employee_id = bo.id
