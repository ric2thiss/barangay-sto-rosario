...
try {
    ...
    $sql = "SELECT a.employee_id AS employee_id, COUNT(*) AS log_count,
            ...
            LEFT JOIN `_tmp_residents` r ON r.id = e.resident_id
            WHERE {$delFilterA}a.created_at >= ? AND a.created_at <= ?
...
$visitorAgeCounts = [];
try {
    $delFilterV = SchemaColumnCache::visitorLogsHasDeletedAt() ? 'deleted_at IS NULL AND ' : '';
    $sql = "SELECT bracket, COUNT(*) AS cnt FROM (
        SELECT
            ...
            END AS bracket
        FROM visitor_logs
        WHERE {$delFilterV}created_at >= ? AND created_at <= ?
    ) t
    GROUP BY bracket
