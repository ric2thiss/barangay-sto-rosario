...
try {
    ...
    // 1) Check profiling-system residents table via ProfilingPdo
    try {
        $profilingPdo = ProfilingPdo::get();
        if ($profilingPdo) {
            $stmt = $profilingPdo->query("SELECT COUNT(*) FROM `residents`");
            $count = (int) $stmt->fetchColumn();
            $result["checks"]["profiling_residents_readable"] = true;
            $result["details"]["profiling_residents_count"] = $count;
        } else {
            $result["details"]["profiling_residents_error"] = "ProfilingPdo connection failed";
        }
    } catch (Exception $e) {
