...
/**
 * For analytics JOINs, we populate a temporary local table with resident demographics
 * from ProfilingPdo, so all SQL can run on the attendance DB without cross-DB syntax.
 */
try {
    $db = (new Database())->connect();

    // Create temp table and populate from profiling DB
    ...
    if ($profilingPdo) {
        $resStmt = $profilingPdo->query("
            SELECT id, first_name, surname, sex, purok, barangay, civil_status, birthdate
            FROM `residents`
        ");
        $resRows = $resStmt->fetchAll(PDO::FETCH_ASSOC) ?: [];
        if (!empty($resRows)) {
            $insertSql = "INSERT INTO `_tmp_residents` (id, first_name, surname, sex, purok, barangay, civil_status, birthdate)
                          VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
            $insStmt = $db->prepare($insertSql);
            foreach ($resRows as $rr) {
                $insStmt->execute([
                    $rr['id'], $rr['first_name'] ?? '', $rr['surname'] ?? '',
                    $rr['sex'] ?? null, $rr['purok'] ?? null, $rr['barangay'] ?? null,
                    $rr['civil_status'] ?? null, $rr['birthdate'] ?? null,
                ]);
            }
        }
    }
} catch (Throwable $e) {
    // If temp table creation fails, queries will still run but demographics will show as Unknown
    error_log('visitor-analytics: temp table populate failed: ' . $e->getMessage());
}

$residentsTable = '`_tmp_residents`';
