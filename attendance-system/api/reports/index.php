...
try {
    ...
    $db->exec("CREATE TEMPORARY TABLE IF NOT EXISTS `_tmp_barangay_official` (
        `id` INT PRIMARY KEY,
        `first_name` VARCHAR(100),
        `middle_name` VARCHAR(100),
        `surname` VARCHAR(100),
        `position` VARCHAR(100),
        `chairmanship` VARCHAR(100)
    ) ENGINE=MEMORY");
    
    $db->exec("CREATE TEMPORARY TABLE IF NOT EXISTS `_tmp_residents` (
        `id` INT PRIMARY KEY,
        `first_name` VARCHAR(100),
        `middle_name` VARCHAR(100),
        `surname` VARCHAR(100),
    ...
    if ($profilingPdo) {
        // Populate officials
        $offStmt = $profilingPdo->query("SELECT id, first_name, middle_name, surname, position, chairmanship FROM `barangay_official`");
        $offRows = $offStmt->fetchAll(PDO::FETCH_ASSOC) ?: [];
        if (!empty($offRows)) {
            $insOff = $db->prepare("INSERT INTO `_tmp_barangay_official` (id, first_name, middle_name, surname, position, chairmanship) VALUES (?, ?, ?, ?, ?, ?)");
            foreach ($offRows as $row) {
                $insOff->execute([$row['id'], $row['first_name'] ?? null, $row['middle_name'] ?? null, $row['surname'] ?? null, $row['position'] ?? null, $row['chairmanship'] ?? null]);
            }
        }
        
        // Populate residents
        $resStmt = $profilingPdo->query("SELECT id, first_name, middle_name, surname, occupation FROM `residents`");
        $resRows = $resStmt->fetchAll(PDO::FETCH_ASSOC) ?: [];
        if (!empty($resRows)) {
            $insRes = $db->prepare("INSERT INTO `_tmp_residents` (id, first_name, middle_name, surname, occupation) VALUES (?, ?, ?, ?, ?)");
            foreach ($resRows as $row) {
                $insRes->execute([$row['id'], $row['first_name'] ?? null, $row['middle_name'] ?? null, $row['surname'] ?? null, $row['occupation'] ?? null]);
            }
        }
    }
    ...
    switch ($type) {
        case 'attendance-position':
            ...
            $query = "
                ...
                FROM attendances a
                LEFT JOIN `_tmp_barangay_official` ps_off ON a.employee_id = ps_off.id
                LEFT JOIN `_tmp_residents` ps_res ON a.employee_id = ps_res.id
