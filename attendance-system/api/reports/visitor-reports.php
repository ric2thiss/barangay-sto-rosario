...
try {
    ...
    switch ($type) {
        ...
        case 'gender-distribution':
            // Gender distribution (join with profiling-system residents for resident visitors via temp table)
            require_once __DIR__ . '/../../app/utils/ProfilingPdo.php';
            
            $db->exec("CREATE TEMPORARY TABLE IF NOT EXISTS `_tmp_residents_gender` (
                `id` INT PRIMARY KEY,
                `sex` VARCHAR(20)
            ) ENGINE=MEMORY");
            ...
            if ($profilingPdo) {
                $resStmt = $profilingPdo->query("SELECT id, sex FROM `residents`");
                $resRows = $resStmt->fetchAll(PDO::FETCH_ASSOC) ?: [];
                if (!empty($resRows)) {
                    $insertSql = "INSERT INTO `_tmp_residents_gender` (id, sex) VALUES (?, ?)";
                    $insStmt = $db->prepare($insertSql);
                    foreach ($resRows as $rr) {
                        $insStmt->execute([$rr['id'], $rr['sex'] ?? null]);
                    }
                }
            }
            ...
            $query = "
                ...
                LEFT JOIN `_tmp_residents_gender` r ON vl.resident_id = r.id AND vl.is_resident = 1
