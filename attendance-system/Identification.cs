...
namespace UareUSampleCSharp
{
    ...
    {
        // ⚠️ CONFIGURATION: Change these URLs to match your server environment
        private const string API_BASE_URL = "http://localhost/attendance-system";
        private const string WEBSOCKET_URL = "ws://localhost:8081";
        ...
        private async void OnCaptured(CaptureResult captureResult)
        {
            try
            {
                ...
                using (var client = new HttpClient())
                {
                    ...
                    var response = await client.GetAsync(API_BASE_URL + "/api/templates/index.php");
                    ...
                    foreach (var item in storedTemplates)
                    {
                        try
                        {
                                ...
                                compareResult.Score < (0x7fffffff / 100000))
                            {
                                ...
                                var timeWindowResponse = await client.GetAsync(API_BASE_URL + "/api/attendance/windows.php");
                                ...
                                var logResponse = await client.PostAsync(API_BASE_URL + "/api/attendance/index.php", attendanceContent);
                                ...
                                else if (logResponse.IsSuccessStatusCode)
                                {
                                    ...
                                    if (wsClient != null && wsClient.State == WebSocketState.Open)
                                    {
                                        ...
                                        // OLD ENDPOINT (backward compatible): http://localhost/attendance-system/api/services.php?resource=attendances
                                        // NEW ENDPOINT: http://localhost/attendance-system/api/attendance/index.php
                                        var attendanceDataResponse = await client.GetAsync(API_BASE_URL + "/api/attendance/index.php");
        ...
        private async Task ConnectWebSocketAsync()
        {
            try
            {
                ...
                await wsClient.ConnectAsync(new Uri(WEBSOCKET_URL), CancellationToken.None);
