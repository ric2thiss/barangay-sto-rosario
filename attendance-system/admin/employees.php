...
require_once __DIR__ . "/../app/utils/ProfilingPdo.php";
...
if ($profilingPdo) {
    try {
        $stmt = $profilingPdo->prepare("
            SELECT DISTINCT bo.chairmanship
            FROM `barangay_official` AS bo
...
// Total count for pagination and Page records
$totalRecords = 0;
$employees = [];

if ($profilingPdo) {
    $countSql = "SELECT COUNT(*) as total FROM `barangay_official` AS bo{$whereSql}";
    $countStmt = $profilingPdo->prepare($countSql);
    ...
    ";
    $listStmt = $profilingPdo->prepare($listSql);
    foreach ($params as $key => $val) {
...
<body>
    ...
    <div class="flex min-h-screen">
        ...
        <main class="flex-1 md:ml-64 p-6 transition-all duration-300">
            ...
            <header class="mb-6">
                <div class="flex justify-between items-center mb-1">
                    <div>
                        ...
                        <p class="text-gray-500 text-sm"><?= getGreeting($userName) ?> - Manage all current and past
                            employees in one place.</p>
                    </div>
                ...
                <div class="flex flex-col sm:flex-row justify-between items-center mb-6 space-y-4 sm:space-y-0">
                    ...
                    <div class="flex flex-col sm:flex-row gap-2 w-full sm:w-auto">
                        <form method="GET" action=""
                            class="relative flex-1 sm:w-96 lg:w-[500px] flex items-center gap-2" id="searchForm">
                            ...
                            <?php if (!empty($filters['chairmanship'])): ?>
                                <input type="hidden" name="chairmanship"
                                    value="<?= htmlspecialchars($filters['chairmanship']) ?>">
                            <?php endif; ?>
                        ...
                        <button type="button" id="filterButton"
                            class="px-4 py-2 border border-gray-300 rounded-lg text-gray-700 font-medium hover:bg-gray-50 transition-colors flex items-center gap-2 whitespace-nowrap">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                    ...
                                                    <?php else: ?>
                                                        <a href="biometric-registration.php?search=<?= urlencode((string) ($employee->employee_id ?? '')) ?>"
                                                            ...
                                                            <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor"
                                                                viewBox="0 0 24 24">
                                                                <path stroke-linecap="round" stroke-linejoin="round"
                                                                    stroke-width="2" d="M12 4a6 6 0 00-6 6v3a2 2 0 01-2 2h0"></path>
                                                                <path stroke-linecap="round" stroke-linejoin="round"
                                                                    stroke-width="2" d="M18 10v3a6 6 0 01-6 6h-1"></path>
                                                            </svg>
                                                            Enroll
                                                        </a>
                                                    <?php endif; ?>
                ...
                <div class="mt-6 flex flex-col sm:flex-row justify-between items-center gap-4 text-sm text-gray-600">
                    <div>
                        Showing <span class="font-medium"><?= $startRecord ?></span> to <span
                            ...
                            class="font-medium"><?= $totalRecords ?></span> records
                    ...
                    <?php if ($totalPages > 1): ?>
                        <nav class="flex space-x-1" aria-label="Pagination">
                            ...
                            <?php else: ?>
                                <span
                                    class="px-3 py-2 border border-gray-300 rounded-lg text-gray-400 cursor-not-allowed">Previous</span>
                            <?php endif; ?>
                            ...
                            if ($startPage > 1): ?>
                                <a href="?page=1<?= $queryString ?>"
                                    class="px-3 py-2 border border-gray-300 rounded-lg hover:bg-gray-100 transition-colors">1</a>
                                <?php if ($startPage > 2): ?>
                            ...
                            <?php for ($i = $startPage; $i <= $endPage; $i++): ?>
                                <?php if ($i == $currentPage): ?>
                                    <span
                                        class="px-3 py-2 border border-gray-300 rounded-lg bg-blue-600 text-white font-medium"><?= $i ?></span>
                                <?php else: ?>
                                    <a href="?page=<?= $i ?><?= $queryString ?>"
                                        class="px-3 py-2 border border-gray-300 rounded-lg hover:bg-gray-100 transition-colors"><?= $i ?></a>
                                <?php endif; ?>
                            <?php endfor; ?>
                            ...
                            <?php if ($endPage < $totalPages): ?>
                                ...
                                <a href="?page=<?= $totalPages ?><?= $queryString ?>"
                                    class="px-3 py-2 border border-gray-300 rounded-lg hover:bg-gray-100 transition-colors"><?= $totalPages ?></a>
                            <?php endif; ?>
                            ...
                            <?php else: ?>
                                <span
                                    class="px-3 py-2 border border-gray-300 rounded-lg text-gray-400 cursor-not-allowed">Next</span>
                            <?php endif; ?>
                        </nav>
                    <?php endif; ?>
                </div>
            ...
            <!-- Filter Modal -->
            <div id="filterModal" class="fixed inset-0 z-50 hidden overflow-y-auto" aria-labelledby="filter-modal-title"
                role="dialog" aria-modal="true">
                <div class="fixed inset-0 bg-gray-900 bg-opacity-75 transition-opacity" id="filterModalBackdrop"
                    ...
                    <div class="bg-white rounded-xl shadow-2xl w-full max-w-md transition-all transform sm:my-8"
                        ...
                        <form method="GET" action="">
                            ...
                            <div class="p-6 space-y-4">
                                ...
                                <div>
                                    <label for="filter_chairmanship"
                                        class="block text-sm font-medium text-gray-700 mb-2">
                                        Chairmanship (Department)
                                    </label>
                                    <select name="chairmanship" id="filter_chairmanship"
                                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500">
                                        <option value="">All</option>
                                        <?php foreach ($chairmanshipOptions as $opt): ?>
                                            <option value="<?= htmlspecialchars((string) $opt) ?>"
                                                <?= (!empty($filters['chairmanship']) && $filters['chairmanship'] === (string) $opt) ? 'selected' : '' ?>>
                                ...
                                <div>
                                    ...
                                    <select name="position" id="filter_position"
                                        ...
                                        <?php foreach ($positionOptions as $opt): ?>
                                            <option value="<?= htmlspecialchars((string) $opt) ?>"
                                                <?= (!empty($filters['position']) && $filters['position'] === (string) $opt) ? 'selected' : '' ?>>
                                                <?= htmlspecialchars((string) $opt) ?>
                                            </option>
                                        <?php endforeach; ?>
                                    </select>
                            ...
                            <div
                                class="flex flex-col sm:flex-row justify-end p-5 space-y-3 sm:space-y-0 sm:space-x-3 border-t border-gray-200 bg-gray-50 rounded-b-xl">
                                <a href="?"
                                    class="w-full sm:w-auto px-6 py-2 text-center text-sm font-semibold text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-gray-100 transition">
                                    Clear Filters
                                </a>
                                <button type="submit"
                                    class="w-full sm:w-auto px-6 py-2 text-sm font-semibold text-white bg-blue-600 rounded-lg hover:bg-blue-700 transition">
                                    Apply Filters
                                </button>
                            </div>
                        </form>
