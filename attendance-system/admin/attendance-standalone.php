...
try {
    ...
    $activityFilterList = [];
    error_log('attendance-standalone.php activities filter: ' . $e->getMessage());
}
...
<body>
    ...
    <main class="min-h-screen p-6">
        ...
        <header class="mb-6">
            ...
            <!-- Top actions: event dropdown + Attendance Now + Export (same layout as attendance.php) -->
            <div class="flex flex-nowrap items-end justify-end gap-3 overflow-x-auto pb-1 mt-4">
                    ...
                    <button type="button" id="export-modal-btn" class="flex items-center shrink-0 px-4 py-2 bg-gray-200 hover:bg-gray-300 text-gray-800 font-medium rounded-lg transition-colors shadow-md text-sm h-[38px]">
                            ...
                            <div class="block w-full align-middle">
                                ...
                                <thead class="table-header">
                                    <tr>
                                        ...
                                        <th class="px-3 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider activity-col hidden">Activity</th>
                                    </tr>
                                </thead>
                                <tbody id="attendance-table-body" class="bg-white divide-y divide-gray-200">
                                    ...
                                    <tr class="hover:bg-gray-50 transition duration-150">
                                        ...
                                        <td class="px-3 py-3 whitespace-nowrap text-sm text-gray-600 activity-col hidden"><?= htmlspecialchars($attendance->activity_name ?? '—') ?></td>
                                    </tr>
                                    <?php endforeach ?>
                                    <?php endif; ?>
    ...
    <!-- Export Modal -->
    <div id="export-modal" class="fixed inset-0 z-50 hidden bg-gray-900 bg-opacity-50 flex items-center justify-center">
        <div class="bg-white rounded-xl shadow-xl w-full max-w-md p-6 transform transition-all scale-95 opacity-0" id="export-modal-content">
            <div class="flex justify-between items-center mb-5">
                <h3 class="text-lg font-bold text-gray-900">Export Attendance</h3>
                <button type="button" id="export-cancel-x" class="text-gray-400 hover:text-gray-600">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg>
                </button>
            </div>
            <form id="export-form" action="../api/attendance/report-export.php" method="GET" target="_blank">
                ...
                <div class="space-y-4 mb-6">
                    <div>
                        ...
                        <input type="text" name="search" placeholder="Employee name or ID..." class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-blue-500 focus:border-blue-500">
                    </div>
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">From Date</label>
                            <input type="date" name="from" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-blue-500 focus:border-blue-500">
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">To Date</label>
                            <input type="date" name="to" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-blue-500 focus:border-blue-500">
                        </div>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Activity Filter</label>
                        <select name="activity_id" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-blue-500 focus:border-blue-500">
                            <option value="">All Activities</option>
                            ...
                            <?php foreach ($activityFilterList as $act): ?>
                                ...
                                $adate = is_object($act) ? (string) ($act->activity_date ?? '') : (string) ($act['activity_date'] ?? '');
                                ?>
                                <option value="<?= $aid ?>"><?= htmlspecialchars($aname . ' (' . $adate . ')') ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                </div>
                <div class="flex justify-end gap-3">
                    <button type="button" id="export-cancel-btn" class="px-4 py-2 bg-gray-100 hover:bg-gray-200 text-gray-700 font-medium rounded-lg transition-colors text-sm">Cancel</button>
                    <button type="submit" id="export-confirm-btn" class="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white font-medium rounded-lg transition-colors shadow-sm text-sm">Export CSV</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const openBtn = document.getElementById('export-modal-btn');
            const closeBtns = [document.getElementById('export-cancel-x'), document.getElementById('export-cancel-btn')];
