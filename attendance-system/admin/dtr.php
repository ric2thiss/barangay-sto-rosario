...
try {
    ...
    // Get officials from profiling DB
    $officialMap = [];
    if ($profilingPdo) {
        $offStmt = $profilingPdo->query("
            SELECT CAST(id AS CHAR) AS employee_id, first_name, middle_name, surname AS last_name
            FROM `barangay_official`
            ORDER BY surname ASC, first_name ASC
        ");
        foreach ($offStmt->fetchAll(PDO::FETCH_ASSOC) as $off) {
            $officialMap[$off['employee_id']] = $off;
        }
    }

    // Merge: all official IDs + local IDs
    $allIds = array_unique(array_merge(array_keys($officialMap), $localIds));

    $employees = [];
    foreach ($allIds as $eid) {
        $off = $officialMap[$eid] ?? null;
        $employees[] = [
            'employee_id' => $eid,
    ...
    // Sort: named officials first, then by last name
    usort($employees, function ($a, $b) {
        $aHasName = !empty($a['last_name']);
        $bHasName = !empty($b['last_name']);
