...
export async function initActivitySelect(optionsUrl, activeUrl) {
    ...
    opts.forEach((o) => {
        ...
        const tag = (o.source === 'PSS' || o.source === 'LGUMS') ? 'PSS' : 'Local';
    ...
    function toggleActivityColumn(val) {
        const hasActivity = val !== null && String(val) !== '' && String(val) !== '0';
        document.querySelectorAll('.activity-col').forEach(el => {
            if (hasActivity) {
    ...
    toggleActivityColumn(sel.value);
