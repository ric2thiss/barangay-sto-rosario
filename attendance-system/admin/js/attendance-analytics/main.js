...
/**
 * Read the inline table filter values. Falls back to global filters when inline fields are empty.
 */
function getTableFilterParams() {
    const baseParams = getFilterParamsForApi();

    // Inline search narrows to a specific employee name (client-sent as search param)
    const tableSearch = (document.getElementById('table-search')?.value || '').trim();

    // Inline date range overrides global when filled
    const tFrom = document.getElementById('table-date-from')?.value || '';
    const tTo = document.getElementById('table-date-to')?.value || '';
    if (tFrom) baseParams.from = tFrom;
    if (tTo) baseParams.to = tTo;

    // Inline status overrides the global status filter
    const tStatus = document.getElementById('table-status')?.value || 'all';
    // Inline per page
    const tPerPage = parseInt(document.getElementById('table-per-page')?.value || '25', 10) || 25;

    return {
        ...baseParams,
        status: tStatus,
        per_page: Math.min(100, Math.max(1, tPerPage)),
        page: state.page,
...
/**
 * Sync inline table filters from the global panel (first load / global Refresh).
 */
function syncTableFiltersFromGlobal() {
    const gFrom = document.getElementById('filter-from')?.value || '';
    const gTo = document.getElementById('filter-to')?.value || '';
    const gStatus = document.getElementById('filter-status')?.value || 'all';
    const gPerPage = document.getElementById('filter-per-page')?.value || '25';

    const tFrom = document.getElementById('table-date-from');
    const tTo = document.getElementById('table-date-to');
    const tStatus = document.getElementById('table-status');
    const tPerPage = document.getElementById('table-per-page');

    if (tFrom && !tFrom.value) tFrom.value = gFrom;
    if (tTo && !tTo.value) tTo.value = gTo;
    if (tStatus) tStatus.value = gStatus;
    if (tPerPage) tPerPage.value = gPerPage;
}

async function load() {
    syncTableFiltersFromGlobal();

    const params = getTableFilterParams();
...
/**
 * Load ONLY the table (no charts/summary refresh) — used by inline table filters.
 */
...
function init() {
    ...
    document.getElementById('btn-refresh')?.addEventListener('click', () => {
        // Global refresh: sync inline filters from global, then full load
        const tFrom = document.getElementById('table-date-from');
        const tTo = document.getElementById('table-date-to');
        if (tFrom) tFrom.value = '';
        if (tTo) tTo.value = '';
        document.getElementById('table-search') && (document.getElementById('table-search').value = '');
        resetToFirstPage();
        load();
    });
    document.getElementById('filter-period')?.addEventListener('change', () => {
        ...
        // Clear inline date fields too so they resync
        const tFrom = document.getElementById('table-date-from');
        const tTo = document.getElementById('table-date-to');
        if (tFrom) tFrom.value = '';
        if (tTo) tTo.value = '';
    ...
    // Inline table filter controls
    document.getElementById('table-apply-filters')?.addEventListener('click', () => {
        resetToFirstPage();
        loadTableOnly();
    });
    ['table-status', 'table-per-page'].forEach((id) => {
        document.getElementById(id)?.addEventListener('change', () => {
            resetToFirstPage();
            loadTableOnly();
        });
    });
    // Enter key in search triggers table reload
    document.getElementById('table-search')?.addEventListener('keydown', (e) => {
        if (e.key === 'Enter') {
            e.preventDefault();
            resetToFirstPage();
            loadTableOnly();
