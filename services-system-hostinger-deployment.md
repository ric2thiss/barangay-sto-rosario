...
# Livewire Subdirectory Fix (Use full URL if relative fails)
LIVEWIRE_ASSET_URL=https://sr-magallanes.com/services-system
...
### A. 403 Forbidden Error (Livewire Assets)
If you see a 403 error for `/vendor/livewire/livewire.js` in the console:

1.  **Check the URL**: If your browser address bar shows `/public/` (e.g., `.../services-system/public/login`), you are accessing the site incorrectly. Use `https://sr-magallanes.com/services-system/` directly. The root `.htaccess` handles the rest.
2.  **ModSecurity**: Hostinger's security firewall often blocks the `vendor` folder name.
    -   Log in to **hPanel** > **Security** > **ModSecurity**.
    -   Try disabling it temporarily to see if the error disappears.
3.  **Publish Assets (Recommended)**: For production on shared hosting, it is often better to have physical files.
    -   Run `php artisan livewire:publish --assets` locally.
    -   Upload the `public/vendor` folder to your server's `public_html/services-system/public/vendor`.
4.  **Permissions**: Ensure the `public/vendor` folder on the server has **755** permissions and files inside have **644**.

### B. Login Not Responding / No Error Messages
This is almost always caused by the Livewire JavaScript failing to load (the 403 error above). Since the login button is a Livewire component, it won't trigger if the JS is blocked. Fix the 403 error first, and the login will start working.

### C. Asset Pathing Fix
If images or CSS are still showing `/public/` in the URL:
-   Double-check that `APP_URL` in `.env` **does not** end with a slash.
-   Ensure the root `.htaccess` is exactly as shown in Section 3B.
