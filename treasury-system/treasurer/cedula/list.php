...
if ($searchQuery !== "") {
    ...
    $stmt = $conn->prepare("
        SELECT c.*, COALESCE(c.issued_date, ps.created_at) as display_date FROM cedula c
        LEFT JOIN payment_status ps ON (c.resident_id = ps.resident_id AND c.resident_id IS NOT NULL AND c.resident_id != 0)
            OR (c.full_name = ps.resident_fname)
        WHERE (c.issued_by IS NOT NULL OR (ps.certificate_type = 'Cedula' AND ps.payment_status = 'paid'))
            AND (
                c.full_name LIKE ?
                OR c.cedula_no LIKE ?
                OR c.tin LIKE ?
        ...
        GROUP BY c.id
        ORDER BY display_date DESC, c.id DESC
    ");
    ...
    $result = $conn->query("
        ...
        LEFT JOIN payment_status ps ON (c.resident_id = ps.resident_id AND c.resident_id IS NOT NULL AND c.resident_id != 0)
            OR (c.full_name = ps.resident_fname)
        WHERE c.issued_by IS NOT NULL 
           OR (ps.certificate_type = 'Cedula' AND ps.payment_status = 'paid')
        GROUP BY c.id
        ORDER BY display_date DESC, c.id DESC
    ");
}
                            ...
                            <tbody>
                                <?php if ($result->num_rows > 0): ?>
                                    <?php while ($row = $result->fetch_assoc()): ?>
                                        <tr>
                                            <td><?= date('M d, Y', strtotime($row['display_date'])) ?>
                                            </td>
