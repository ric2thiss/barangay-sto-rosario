    ...
    <script>
        function numberToWords(num) {
            ...
            function convertInteger(n) {
                ...
                if (n < 1000000) return convertInteger(Math.floor(n / 1000)) + ' thousand' + (n % 1000 !== 0 ? ' ' + convertInteger(n % 1000) : '');
                if (n < 1000000000) return convertInteger(Math.floor(n / 1000000)) + ' million' + (n % 1000000 !== 0 ? ' ' + convertInteger(n % 1000000) : '');
                return '';
            }

            let str = convertInteger(Math.floor(num));
            str = str.charAt(0).toUpperCase() + str.slice(1) + ' pesos';
            
            let cents = Math.round((num - Math.floor(num)) * 100);
            if (cents > 0) {
                str += ' and ' + cents + '/100 centavos';
            }
            return str;
        }
        ...
        function computeTotals() {
            ...
            if (wordsField) {
                wordsField.value = numberToWords(total);
            }
        }
        ...
        nameInput.addEventListener('input', function() {
            ...
            debounceTimer = setTimeout(() => {
                fetch(`get_profiling_people.php?search=${encodeURIComponent(searchTerm)}`)
                    .then(response => response.json())
                    .then(data => {
                        if (data.length > 0) {
                            suggestionsDiv.innerHTML = data.map(person => {
                                let sourceLabel = 'Payment record';
                                if (person.source === 'cedula') sourceLabel = 'Has cedula record';
                                if (person.source === 'profiling') sourceLabel = 'Resident Profiling Record';
                                
                                return `<div class="suggestion-item" style="padding: 10px; cursor: pointer; border-bottom: 1px solid #eee;" data-name="${person.name}">
                                    <i class="fas fa-user"></i> ${person.name}
                                    <small style="color: #666; margin-left: 10px;">(${sourceLabel})</small>
                                </div>`;
                            }).join('');
                                    ...
                                    fetch(
                                        ...
                                        .then(personData => {
                                            if (!personData.error) {
                                                ...
                                                if (personData.annual_income !== null && personData.annual_income !== undefined) {
                                                    document.getElementById('annual_income').value = personData.annual_income;
                                                    calculateAdditionalFromIncome();
                                                }
                                                ...
                                                if (personData.resident_id) {
                                                    document.getElementById('resident_id').value = personData.resident_id;
                                                }
                                });
