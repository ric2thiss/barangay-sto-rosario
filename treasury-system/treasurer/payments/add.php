...
// Fetch service types from services-system
$servicesStmt = $conn->query("SELECT certificate_type_id, certificate_name, price FROM " . DB_SERVICES . ".certificate_types ORDER BY certificate_name ASC");
$services = [];
if ($servicesStmt) {
    while ($row = $servicesStmt->fetch_assoc()) {
        $services[] = $row;
    }
}
?>
...
<body>
        ...
        <div class="card">
          ...
          <form method="POST" action="save.php">
            ...
            <div class="form-group">
              ...
              <input type="hidden" id="payer_source" name="payer_source">
              <div id="suggestions"
            ...
            <div class="form-row">
              <div class="form-group">
                ...
                <select id="service_type" name="service_type" required>
                  <option value="" data-price="0" data-id="">Select Service</option>
                  <?php foreach ($services as $service): ?>
                    <option value="<?= htmlspecialchars($service['certificate_name']) ?>" 
                            data-id="<?= htmlspecialchars($service['certificate_type_id']) ?>"
                            data-price="<?= htmlspecialchars($service['price']) ?>">
                        <?= htmlspecialchars($service['certificate_name']) ?>
                    </option>
                  <?php endforeach; ?>
                </select>
            ...
            <div class="form-row">
              <div class="form-group">
                ...
                <input type="number" id="amount" name="amount" step="0.01" min="0" placeholder="0.00" required readonly style="background: #e9ecef;">
              </div>
            ...
            <div class="form-actions" style="display: flex; gap: 12px; margin-top: 10px;">
              ...
              <a href="list.php" class="btn btn-secondary" style="flex: 1; justify-content: center;">
  ...
  <script>
    ...
    document.getElementById('service_type').addEventListener('change', function() {
        ...
        const certTypeId = selectedOption.getAttribute('data-id') || '';
        
        document.getElementById('amount').value = price.toFixed(2);
        document.getElementById('certificate_type_id').value = certTypeId;
        calculateTotal();
    });
          ...
          .then(data => {
            if (data.length > 0) {
              suggestionsDiv.innerHTML = data.map(person =>
                `<div class="suggestion-item" style="padding: 10px; cursor: pointer; border-bottom: 1px solid #eee;" data-name="${person.name}" data-id="${person.id}" data-source="${person.source}">
              ...
              document.querySelectorAll('.suggestion-item').forEach(item => {
                item.addEventListener('click', function() {
                  ...
                  document.getElementById('payer_source').value = source;
