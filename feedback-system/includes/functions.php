...
function updateAssignmentStatus($conn, $assignment_id, $status, $admin_notes = null)
{
    ...
    // If resolved, update personnel rating and feedback status
    if ($success) {
        // Get assignment and personnel details
        $get_sql = "SELECT fa.feedback_id, fa.personnel_id, f.rating FROM feedback_assignments fa 
        ...
        if ($assignment) {
            ...
            // Sync the parent feedback status
            if ($status == 'Resolved') {
                $update_feedback_sql = "UPDATE feedback SET is_resolved = 1, resolved_at = NOW() WHERE id = ?";
            } else {
                $update_feedback_sql = "UPDATE feedback SET is_resolved = 0, resolved_at = NULL WHERE id = ?";
            }
            $uf_stmt = $conn->prepare($update_feedback_sql);
            $uf_stmt->bind_param("i", $feedback_id);
            $uf_stmt->execute();

            // Update personnel stats if newly resolved
            if ($status == 'Resolved' && !empty($assignment['personnel_id'])) {
 ...
 * Log Admin Activity
 */
function logAdminActivity($conn, $admin_id, $action, $details = null)
{
    if (!$conn || !$admin_id)
        return false;

    $ip_address = $_SERVER['REMOTE_ADDR'];
    $sql = "INSERT INTO activity_logs (admin_id, action, details, ip_address) VALUES (?, ?, ?, ?)";
    $stmt = $conn->prepare($sql);
    if ($stmt) {
